...
fundamentals fact, a portfolio account, or a recorded trade, so the
`fx_usd_daily` view (migration 0007) can convert dashboard values into any
display currency. New pairs backfill their full history on the next price
fetch, exactly like any other instrument.
"""
...
from fintracker.models import Account, Fundamental, Instrument, PortfolioTransaction
...
def ensure_fx_instruments() -> int:
    ...
    with session_scope() as session:
        ...
        # Portfolio trades may settle in a currency nothing else mentions.
        currencies |= set(session.scalars(select(Account.currency).distinct()))
        currencies |= set(session.scalars(select(PortfolioTransaction.currency).distinct()))
